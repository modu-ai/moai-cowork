...
description: |
  ...
  2개 동작 모드가 자연어로 자동 선택됩니다 — ① 리뷰 집계 모드(4채널 리뷰 → 감정·키워드·인사이트·액션플랜 4단 분석) ② VOC 분류·트리아지 모드(고객 핏·빈도·핵심 가치 3축 × KTAS 5단계 응급도 → 처리 순서 + 응답 템플릿). 텍스트 산출물은 근거와 어조를 자체 검수하고, 다른 플러그인의 표현 검수 스킬이 설치돼 있으면 추가로 사용합니다.
...
|------|------|---------|---------|------|
| **Level 1** | 🔴 즉시 (Resuscitation) | 27 | <1시간 | 중대한 환불 분쟁 / 안전·규제 신고 |
...
- **Level 1**: 담당자에게 즉시 이관하고 문제를 인정하는 초안을 준비한다. 환불·교환·보상과 연락 시점은 셀러가 결정한 뒤에만 고객에게 약속한다.
- **Level 2**: CS팀장 응답 + 1:1 솔루션 + 후속 follow-up
- **Level 3**: CS 표준 응답 초안과 담당자 확인 항목 작성
- **Level 4**: 자동 응답 후보 작성 후 담당자 정책 확인
- **Level 5**: 자동 응답 + 감사 표현
...
두 모드의 텍스트 산출물(리뷰 집계 모드의 인사이트·액션플랜·서비스 평가, VOC 분류 모드의 응답 템플릿 Level 1-5·처리 순서 안내)은 원자료와 수치를 대조하고, 미승인 약속·개인정보·과장·경어를 이 스킬 안에서 검수합니다. `moai-coworker:ai-slop-reviewer`가 현재 설치돼 사용 가능하면 표현을 추가 검수합니다.
...
위 처리 시한은 셀러가 실제로 운영하는 SLA를 확인한 뒤 적용한다. 위험·법적 시한·결제 장애는 고객 등급 점수보다 먼저 검토한다. 이 스킬의 결과는 분류와 초안이며, 고객 발송·환불 승인·보상 결정이 완료됐다는 뜻이 아니다.
