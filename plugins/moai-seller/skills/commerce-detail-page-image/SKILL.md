...
호스트에서 실제 사용 가능한 이미지 생성 도구로 섹션 이미지를 만듭니다.
Python과 Pillow가 실행 가능할 때만 1080×12720 단일 합성 이미지를 산출하며,
실행하지 못한 합성은 미완료로 보고합니다. 합성본은 작업용 원본이며,
판매 채널에 올리기 전 현재 계정의 이미지 규격과 길이·용량 제한을 확인합니다.
아래 인라인 코드는 작업용 합성까지만 수행하며, 채널별 변환이나 업로드는
수행하지 않습니다. 규격이 다르면 변환 방법과 필요한 도구를 안내하고
변환·업로드 결과가 확인되기 전에는 `업로드 준비 완료`로 표시하지 않습니다.
