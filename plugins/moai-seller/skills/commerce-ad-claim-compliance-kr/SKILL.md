...
description: |
  [책임 경계] 광고·상세페이지·리스팅 "문구·표현" 법규 검증 게이트 전담 — ① 표시광고법 부당표시 6유형 체크리스트 ② 식약처 건강기능식품·화장품 의약품-오인 표현 금지 목록 ③ 전자상거래법 필수 고지(청약철회 7일·통신판매업 신고번호·사업자정보·배송/반품 비용) 점검. 페어 commerce-message-compliance-kr(정통망법 메시지 발송 규제)와 명확히 구분 — 본 스킬은 카피의 표현 적법성, 페어는 발송 채널·시간·동의 적법성.
  ...
  입력 카피 → 유형별 위험 신호 검사 → 확인한 근거와 수정안 제시. 제품 인허가·증빙·현행 법령이 확인되지 않으면 적법성 PASS를 선언하지 않습니다.
...
**책임 한 줄**: 광고 카피(또는 상세페이지 전문) + 상품 카테고리 입력 → 3축 위험 신호 검사 → [문구 | 확인할 사항 | 근거 | 수정안] 표. 자료가 없으면 적법성 PASS를 선언하지 않습니다.

**페어 분리**: `commerce-message-compliance-kr`는 메시지 **발송**(정통망법 스팸 규정 — 옵트인·야간·수신거부) 전담이고, 본 스킬은 **문구·표현** 전담입니다. CRM 광고 메시지는 두 게이트를 모두 통과해야 합니다.
...
기능성 원료의 인정 문구·1일 섭취량은 현재 식약처 공식 자료와 제품별 인정 서류를 대조합니다. `moai-lawyer:legal-mfds-safety`가 설치되고 실제 조회가 가능하면 보조로 사용합니다. 조회나 서류 대조가 불가능하면 해당 표현의 적합성은 미확인으로 남기며 PASS로 표시하지 않습니다.
...
| 3 | 통신판매업 신고 여부와 신고번호 표시 | 전상법 제12조·제13조 | 신고 대상인지, 제12조 신고 예외에 해당하는지 먼저 확인. 신고 대상인 경우에만 번호 표시를 점검. 확인 불가 시 미확인으로 기록 |
...
> 반품비와 청약철회 제한 사유는 상품·계약·실제 고지 화면을 함께 확인합니다. 하자·오배송과 단순 변심을 구분하고, 현행 조문과 적용 예외를 확인합니다.
...
[Step 5] 검토 결과 + 수정안
   ├─ 근거와 사실관계가 확인된 문제 → 발행 보류 + 문구별 수정안
   ├─ 자료·고지 화면·인정 문구 미확인 → 확인 필요
   └─ 검토한 범위에서 문제 미발견 → 검토 범위와 미확인 항목 명시
```
...
- 통신판매업 신고 대상 여부 미확인 — 전상법 제12조의 신고 예외 해당 여부를 먼저 확인하고, 신고 대상인 경우에만 표시 상태를 점검
- 반품 배송비 금액 미고지 — 전상법 제13조 (예: "단순 변심 반품비 3,000원, 하자 시 판매자 부담")

### 재검 안내

    수정안 반영 후 근거 자료와 현재 고지 화면을 다시 대조하십시오.
```
...
→ 확인 필요: 해당 제품의 인정 원료·기능성 문구·섭취량과 실제 광고 문구 대조
...
- **법령 개정**: 조문 번호·기준은 개정될 수 있으므로 산출물에 점검 기준일을 명기하고, 연결된 법령 도구 또는 국가법령정보센터에서 현행 조문을 확인합니다. 도구가 연결되지 않았으면 직접 확인할 출처를 제시하고 미확인으로 표시합니다.
...
- `commerce-message-compliance-kr` — 정통망법 발송 규제 게이트 (페어: 발송 vs 문구)
