...
description: |
  [책임 경계] 한국 정보통신망법(정통망법) 스팸 규정 전담 — 메시지 "발송" 규제만 다룹니다. 광고 "문구·표현"(표시광고법 부당표시, 식약처 의약품 오인, 전상법 필수 고지)은 moai-seller:commerce-ad-claim-compliance-kr 담당.
  광고·정보성 메시지 자동 판정 + 야간 발송 차단 + 표기 위치 검증 + 무료 수신거부 명시 점검 전담. 모든 채널(SMS/LMS/MMS·이메일·앱푸시·카톡 친구톡/알림톡·텔레마케팅) 발송 전 의무 통과 법규 게이트. 페어 스킬 cs-channel-message(메시지 카피 작성)와 명확히 구분 — 본 스킬은 발송 전 사전 점검, 페어는 카피 생성.
...
# 정보통신망법 광고·정보성 메시지 준수 게이트 (Korean Message Compliance)
...
영리 목적의 광고성 정보를 전송할 때는 원칙적으로 **수신자의 사전 명시적 동의**가 필요하다. 제50조 제1항의 법정 예외를 주장한다면 실제 거래관계·재화의 종류·거래 종료일 등 요건을 자료로 확인해야 한다. 예외 입증이 없으면 광고 동의 미확보로 차단한다.
...
오후 9시부터 다음 날 오전 8시까지의 광고성 정보 전송은 **별도 사전 동의**가 필요하다. 시행령 제61조 제2항에 따라 **전자우편은 이 야간 별도 동의 요건의 예외**다.
...
| 채널 | 야간(21:00~08:00) 광고 전송 조건 | 광고 동의·예외 확인 |
|------|-------------------|-----------|
| SMS/LMS/MMS | 별도 야간 사전 동의 확인 | 광고 동의 또는 법정 예외 확인 |
| 이메일 | 야간 별도 동의 요건의 예외 | 광고 동의 또는 법정 예외 확인 |
| 앱푸시 (광고성) | 별도 야간 사전 동의 확인 | 광고 동의 또는 법정 예외 확인 |
| 앱푸시 (정보성) | 광고 야간 제한 적용 대상 아님 | 실제 정보성인지 확인 |
| 카카오 광고 메시지 | 별도 야간 사전 동의와 현재 채널 정책 확인 | 광고 동의·채널 요건 확인 |
| 카카오 알림톡 | 광고 전송 불가; 정보성 승인 템플릿만 | 광고 내용 혼합 여부 확인 |
| 텔레마케팅 | 해당 매체의 현행 규정 별도 확인 | 전화 권유 규정 별도 확인 |
...
다음 중 하나라도 해당하면 **광고성**으로 분류 — 동의 또는 입증된 법정 예외·광고 표시·매체별 야간 요건·무료 수신거부를 점검한다:
...
[Step 1] 광고성 vs 정보성 판정
   ...
   ├─ 광고성 + 카카오 알림톡 → BLOCK (정보성 승인 템플릿만 발송 가능)
   └─ 그 외 광고성 → Step 2

[Step 2] 옵트인 동의 점검
   ├─ 광고 동의 ❌ + 법정 예외 입증 ❌ → BLOCK ("사전 동의 또는 입증된 예외 없음")
   ├─ 광고 동의 ❌ + 법정 예외 입증 ✅ → Step 3 (입증 자료 기록)
   └─ 광고 동의 ✅ → Step 3

[Step 3] 발송 시간 점검 (21시~익일 8시)
   ├─ 이메일 → Step 4 (시행령 제61조 제2항의 야간 별도 동의 예외)
...
```
"/commerce-message-compliance-kr — SMS '오늘만 20% 할인, 카카오페이 결제 시 +5% 적립',
 발송 22:30, 광고 동의 ✅, 야간 동의 ❌"
→ BLOCK: 야간 동의 미확보 + (광고) 표기 누락 + 발신자 정보 누락
   fix 3종 제시

"/commerce-message-compliance-kr — 이메일 '주문하신 비건 세럼 출고되었습니다',
 발송 23:00, 광고 동의 ❌"
→ PASS (정보성, 야간 제한 없음, 옵트인 불필요)
   rendered_message 출력

"/commerce-message-compliance-kr — 카톡 친구톡 '신규 컬렉션 30% 할인',
...
체이닝 순서: `cs-channel-message` (카피 작성) → **commerce-message-compliance-kr** (법규 게이트) → 실제 발송

- `cs-channel-message` — 메시지 카피 작성 + 앱 푸시 4원칙 기획 (이전 단계, 앱 푸시 기획 모드 포함)
- `content-email-sequence` — 이메일 시퀀스 (이메일 채널 카피)

## 이 스킬을 사용하지 말아야 할 때

- **메시지 카피 작성**: `cs-channel-message` 사용
- **광고 캠페인 기획**: `marketing-campaign-planner` 사용
- **광고 문구·표현 심사 (표시광고법·식약처·전상법)**: `commerce-ad-claim-compliance-kr` 사용 — 본 스킬은 발송 규제(정통망법)만 담당
- **글로벌 GDPR·CCPA 준수**: 본 스킬 범위 밖 (한국 정통망법 전담)
- **이메일 시퀀스 자체 작성**: `content-email-sequence` 사용

## 참고 자료

- [정보통신망법 제50조 제3항](https://law.go.kr/lsLinkCommonInfo.do?chrClsCd=010202&lsJoLnkSeq=1030434421) · [같은 법 시행령 제61조 제2항](https://www.law.go.kr/LSW/lsSideInfoP.do?docCls=jo&joBrNo=00&joNo=0061&lsiSeq=288033&urlMode=lsScJoRltInfoR)
- [법제처 거래관계 예외 해석례](https://www.law.go.kr/expcInfoP.do?expcSeq=338973)
- [카카오 알림톡 심사 가이드](https://kakaobusiness.gitbook.io/main/ad/infotalk/audit.md)
- [한국인터넷진흥원(KISA) 불법스팸 방지를 위한 정보통신망법 안내서](https://spam.kisa.or.kr/spam/na/ntt/selectNttInfo.do?bbsId=1002&mi=1020&nttSn=3001)
