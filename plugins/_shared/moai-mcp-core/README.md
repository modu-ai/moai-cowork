...
두 프로세스가 같은 토큰 파일을 정상적으로 쓰는 동안 `.lock` 파일의 운영체제 잠금으로
OAuth 갱신을 직렬화하고, 잠금 안에서 최신 리프레시 토큰을 다시 읽습니다. 잠금 파일을
만들 수 없거나 대기 시간이 지나면 갱신 요청 전에 오류를 돌려줍니다. 저장 실패로
인메모리 폴백 상태가 되면 파일 잠금을 건너뛰므로 여러 앱의 동시 사용을 보장하지
않습니다. 네트워크 파일시스템의 잠금 동작도 별도로 확인해야 합니다.
