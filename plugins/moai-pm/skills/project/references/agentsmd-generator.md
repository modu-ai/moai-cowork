...
- 체인: `moai-marketer:content-blog` → `moai-coworker:ai-slop-reviewer` → `moai-writer:korean-humanize` → 최종 검수 (한국어 산출물)
- 입력: 주제, 키워드, 목표 독자
- 출력: Markdown 포스트 + 검수 리포트 + 정밀 윤문본(등급 A~D) + 최종 검수 판정
```
...
- [ ] **§6-3에 ⟨한국어 감사 3단⟩ 약칭 범례가 있음** — §9 체인 표가 `⟨감사:카피⟩` 같은 약칭을 쓰므로, 범례가 빠지면 생성물에 뜻 모를 기호만 남는다
- [ ] **한국어 품질 체인(§6) 3단이 모두 살아 있음** — 6-1 장르 확정 · 6-2 작성 기준(영어 수사 구조 표) · 6-3 감사 3단 체인(`ai-slop-reviewer` → `korean-spell-check` → `korean-humanize`) — **`korean-humanize`가 마지막이어야 한다.** 순서가 뒤집히면 최종 검수를 통과한 산출물이 전달되지 않는다
- [ ] `{workflow_chains}`의 **한국어 텍스트 산출물 체인마다 감사 3단이 끝에 붙어 있음** — 작성 스킬만 적고 감사를 빠뜨린 체인이 없는지 확인한다
