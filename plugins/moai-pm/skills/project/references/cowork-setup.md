...
한국어 텍스트 산출물 체인은 **반드시 `⟨한국어 감사⟩`로 종료**한다. 비텍스트(차트·데이터·숫자·미디어)는 생략한다.

```
⟨한국어 감사⟩ =
  moai-coworker:ai-slop-reviewer    노출된 경우 일반 슬롭 정리
→ moai-writer:korean-spell-check    노출되고 공개 문서인 경우 맞춤법 제안 수집
→ moai-writer:korean-humanize       노출된 경우 정밀 윤문 + Phase 6 최종 검수
→ 원문·전달본 직접 대조          스킬이 없어도 의미·수치·인용 검수
```

- **[HARD] `korean-humanize`가 호출 가능하면 문장 수정의 마지막 단계다.** Phase 6 최종 검수가 판정한 본문을 다시 고치지 않는다. 없으면 직접 대조하고 사용하지 못한 단계를 기록한다
- **[HARD] 공개로 명시 판정되지 않은 문서는 외부 맞춤법 단계를 건너뛴다.** `korean-spell-check`는 원문을 외부 서비스(`nara-speller.co.kr`)로 전송한다. 계약·NDA·법무 · 재무·세무 · 인사·급여 · 개인정보 · 대외비 · 미공개 사업계획/IR이 해당한다. `korean-humanize`가 노출돼 있으면 맞춤법을 함께 보고, 없으면 직접 확인한다
- 호출 시 옵션을 붙인다 — `장르: 산문|카피|슬라이드`, `최소심각도: S1`. 아래 표의 `⟨감사:카피⟩`·`⟨감사:산문⟩`은 그 장르를 지정하라는 뜻이다
- **작성 스킬만 적고 감사를 빠뜨린 체인을 만들지 않는다.** 윤문은 문장을 다듬을 뿐 목차를 바꾸지 않으므로, 작성 단계 기준(`AGENTS.md` §6-2)과 최종 검수가 **함께** 있어야 한다
...
| 산출물 | 스킬 | 비고 |
|---|---|---|
| 이미지 | ChatGPT Work 기본 이미지 생성 또는 `moai-media:media-higgsfield-image` | 기본 이미지 생성이 노출되면 직접 사용한다. GPT Image 2.5 모델 노출 여부는 확인한다. Higgsfield를 지정한 사용자는 공식 MCP 연결을 확인한다. 실제 생성 결과를 확인한다 |
