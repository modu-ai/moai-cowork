...
4. 이미지가 필요한 경우 ChatGPT에서는 moai-media:media-codex-image로 ChatGPT Images 2.5 생성을 요청한다. 사용자가 Higgsfield를 지정하면 moai-media:media-higgsfield-image로 연결하고, 그때 현재 제공되는 모델을 확인한다. 두 경로의 실행·결과를 섞어 보고하지 않는다. 어느 도구도 사용할 수 없으면 이미지 제작 지시와 원고를 제공하고 이미지 파일 생성 여부를 명시한다.
