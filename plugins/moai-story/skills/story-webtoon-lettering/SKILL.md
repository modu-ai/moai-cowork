...
말풍선·서체·효과음을 담당하는 식자 스킬이다. 영어 스니펫은 이미지 프롬프트의 시작점이며, 모델별 결과를 보고 조정한다. episode(계획)·art(프롬프트 삽입)·qc(가독 판정)가 함께 참조한다.

## 1. 이 스킬이 하는 일

1. **말풍선 선택** — 장면 감정 → 이름 고르기 → 스니펫 복붙. (`references/balloon-dictionary.md`)
2. **서체 결정** — 작품의 기존 서체·브랜드·가독 기준이 있으면 우선하고, 없으면 일관된 기본 서체를 제안한다. (`references/korean-lettering.md`)
...
- **이름과 형태를 함께 적는다** — 말풍선 명칭만으로 생성 결과가 보장되지는 않는다.
...
- **서체 일관성** — 기존 작품 규칙이 있으면 따른다. 별도 규칙이 없으면 본문에 읽기 쉬운 서체를 정해 유지한다.
- **2패스는 결과 확인** — SFX 추가 요청도 기존 그림·말풍선을 바꿀 수 있으므로 원본을 보존하고 결과를 비교한다.
...
> 완성본에서 말풍선을 제거해 클린 원고를 되얻으려면 그림 손상과 비율 변화를 확인한다. 가능하면 원본 클린 컷을 따로 보관한다.
...
| 큰 글씨 / SFX | (2패스) `add SFX only; preserve the existing art and balloons` |
...
2. **2패스** — 완성본을 첨부하고 **SFX만 추가**하도록 요청한다. 결과를 원본과 대조한다.
...
| Post-검수 | `story-webtoon-qc` | SFX 가독 판정 |
