...
description: |
  웹소설 회차 집필 스킬 — 작품의 장르·인물·연재 조건에 맞춰 한 회를 쓰고 끝맺음을 설계한다. 플랫폼이 미정이면 초안의 분량과 게시 조건을 가정으로 표시한다.
...
웹소설 **한 회를 작성**하는 스킬이다. 작품의 목표와 이전 원고에 맞춰 회차를 쓰고, 다음 회로 이어지는 지점을 설계한다. 플랫폼 조건이 확인되지 않았다면 게시용 확정 원고로 표시하지 않는다.

## 1. 개요

웹소설의 결제 방식은 작품과 플랫폼에 따라 다르다. 이 스킬은 **한 회에 무엇을 담을지**와 **어디서 끝낼지**를 함께 다룬다.
...
- **원고 검수 생략 안 함** — 이전 원고·인물 설정·회차 조건을 직접 대조하고, 설치된 문장 검수 스킬은 필요할 때 추가로 쓴다(§7 Post).
...
- `../story-webnovel-planner/references/webnovel-platforms.md` — 게시 대상의 현재 회차 분량·결제 방식 확인 항목.
- `references/genre-craft.md` — 장르별 문법·클리셰 운용.
- `references/cliffhanger-grades.md` — 절단 강도 등급·연참 주기.
- `../story-series-bible/references/recall-design.md` — 재호출 시 이전 원고와 원장을 대조하는 절차.
...
확인한 플랫폼·공모 요건과 작품의 리듬에 맞춰 분량을 조정한다. 요건을 확인하지 못했으면 "확인 필요"로 표시한다.
...
### Post (검수)
- 이전 원고·인물 설정·플랫폼 조건과 회차 내용을 직접 대조한다.
