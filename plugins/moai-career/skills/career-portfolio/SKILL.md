...
채용공고의 평가 기준에 맞춰 문제·본인 기여·검증 가능한 결과를 읽기 쉽게 정리합니다.
...
형식보다 내용의 근거와 가독성을 먼저 점검합니다. 제출 형식은 채용공고를 따릅니다.
...
템플릿이나 커스텀 도메인은 필요할 때 선택합니다. 구성에 걸리는 시간은 준비된 자료와 작업 범위에 따라 다릅니다.
...
프로젝트 기술서·About 단락·케이스 스터디·README 본문 등 포트폴리오의 서술 텍스트는 모두 사람이 읽는 산출물입니다. 아래 외부 스킬은 각각 현재 앱에 설치·노출된 경우에만 해당 단계를 사용합니다. 사용할 수 없는 단계는 추상적 수식어와 상투구를 직접 걷어내고, 프로젝트의 문제·본인 기여·결과를 원자료와 다시 대조합니다.
