...
면담 스크립트와 평가 보고서는 이 스킬에서 실제 평가 기간·기준·행동 근거, 숫자, 개인정보와 미확인 사항을 원자료에 대조합니다. 별도 설치된 `ai-slop-reviewer`나 `korean-humanize`가 현재 앱에 노출될 때만 서술 부분의 문체를 추가 검토하고, 수정 뒤 평가 근거와 의미가 유지됐는지 다시 확인합니다. 표와 대시보드도 산식을 직접 확인합니다. 사용자가 파일 형식을 요청했고 해당 작성 기능이 노출된 경우에만 연결합니다.
