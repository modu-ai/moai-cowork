...
- 공시가격은 **시세가 아님**을 명시한다. 연도·주택 유형에 따른 현실화율을 확인하지 않고 일정 비율로 시세를 추정하지 않는다. 증감률·CAGR을 실거래가 상승률로 해석하지 않도록 경고한다.
...
- **after**: `moai-officer:doc-xlsx` — 노후건물·인허가 파이프라인 엑셀 정리
