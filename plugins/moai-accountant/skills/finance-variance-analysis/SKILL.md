...
- 보험료율 변동은 근로자·사업주 부담분과 적용 연도를 분리해 확인
- **능률 분산** = (실제 근로시간 - 예산 근로시간) × 예산 임금률

**4대보험 비용 분산 체크:** 적용 연도·보수 기준·가입 대상별 실제 보험료를 확인해 예산과 대조한다. 2026년 참고 수치는 `finance-tax-helper/references/2026-rates.md`와 공식 보험기관 자료를 확인한다.
...
업종 평균은 모집단·업종 분류·회계기준·기간이 확인된 공시나 통계가 있을 때만 비교한다. 자료가 없으면 빈칸으로 두고 숫자를 추정하지 않는다.
...
### 분산 분석 활용 원칙
1. **중요성 원칙**: 회사의 실제 중요성 기준과 절대금액·반복성을 함께 확인해 심층 분석 대상을 고른다.
...
- **세율 변경 확인 불가**: 적용 연도의 보험료율과 최저임금을 공식 기관에서 확인할 때까지 인건비 원인을 확정하지 않습니다.
...
- **재무제표 공식 작성**: K-IFRS 기준 재무제표 작성은 `moai-accountant:finance-financial-statements` 스킬을 사용하세요.
- **월말 결산·급여 정산**: 회계 마감 업무는 `moai-accountant:finance-close-management` 스킬이 더 적합합니다.
- **세금 계산**: 세무 관련 문의는 `moai-accountant:finance-tax-helper` 스킬을 사용하세요.
