...
## 2. moai-accountant:finance-financial-statements → mode=financial
...
## 4. moai-officer:productivity-briefing → mode=status (daily variant)

### 적합 모드

`status` 모드 — productivity-briefing의 표준 섹션(헤드라인/업계뉴스/경쟁사/규제/시장지표/액션아이템)이 status 모드의 메트릭카드·하이라이트·Shipped테이블·Velocity차트·Carryover 구조에 잘 대응됨.
...
- productivity-briefing용 status 변형 모드 추가 검토: "status-daily" 또는 status 템플릿에 `news_table` 별도 슬롯 추가.
...
| moai-officer:productivity-briefing | status | 5개 | 3개 | **4** |
