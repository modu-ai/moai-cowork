---
name: meta-skill-tester
description: |
  스킬 품질 검증 절차. 실행 가능한 테스트 케이스를 확인하고 관측한 결과를 기록합니다.
  A/B·회귀·체인 테스트의 기록 방식과 4차원 평가 기준을 제공합니다.

  다음과 같은 요청 시 반드시 이 스킬을 사용하세요:
  - "스킬 테스트해줘", "스킬 검증", "meta-skill-tester"
...
meta-skill-builder로 생성된 스킬 또는 기존 스킬을 검토합니다. 대상 호스트에서 실행할 수 있으면 스킬 사용 전후 결과를 비교하고, 실행하지 못한 검사는 별도로 표시합니다. **4차원 평가 기준**과 **체인 검증 절차**를 이 문서에 담았습니다.
...
3. [실행]    → 사용 가능한 대상 호스트에서 테스트 프롬프트 실행
4. [기록]    → 실행 환경, 입력, 실제 출력과 확인 가능한 측정값 기록
5. [스코어]  → 관측한 결과에만 4차원 루브릭 적용
6. [비교]    → 비교 실행이 있을 때만 baseline과 with-skill 비교
7. [보고]    → PASS / FAIL / NOT-RUN과 미확인 항목 기록
```
...
1. **Baseline 실행**: 대상 호스트에서 스킬 없이 동일 프롬프트 실행
2. **With-Skill 실행**: 같은 호스트·모델·설정에서 스킬을 로드한 뒤 실행
3. 두 실행의 입력, 출력, 도구 호출, 모델과 설정을 기록합니다. 실행할 수 없으면 `NOT-RUN`으로 보고합니다.
4. **비교 측정**:

| 메트릭 | 측정 방법 | 판정 |
|--------|----------|-----------|
| 토큰 사용량 | 호스트가 사용량을 제공할 때만 input + output 토큰 | 관측값과 차이 보고 |
| 출력 품질 | 관측한 결과를 동일 루브릭으로 평가 | 점수와 근거 보고 |
| 정확성 | 실행한 assertion의 통과율 | 사전 정의한 기준과 비교 |
| 완전성 | 필수 출력 포함 여부 | 누락 항목 보고 |

반복 실행 횟수는 결과 변동성과 판단 목적에 맞춰 정합니다. 한 번의 예시는 전체 호스트의 성능을 대표하지 않습니다.
...
1. 수정 전 실제 테스트 결과가 있으면 동일 호스트·설정의 baseline으로 로드
...
| 이전 실행 결과 없음 | NOT-RUN (회귀 비교 불가) |

#### Mode 3: 체인 테스트 (스킬 조합 검증)

여러 스킬이 순차적으로 연결되는 체인을 테스트합니다. 실제로 존재하고 실행 가능한 체인만 실행 대상으로 삼고, 나머지는 미실행으로 기록합니다.
...
| Chain | Steps | Status |
|-------|-------|--------|
| 사업계획서(PPT) | consult-strategy → doc-pptx → ai-slop-reviewer | 예시, 실행 기록 없음 |
| 블로그 발행 | content-blog → ai-slop-reviewer → (optional) media-higgsfield-image | 예시, 실행 기록 없음 |
| 제품 랜딩 | content-copywriting → marketing-landing-page → ai-slop-reviewer | 예시, 실행 기록 없음 |
...
특정 스킬의 `SKILL.md` 본문만 검토할 때는 문서의 명확성·범위만 평가할 수 있습니다. 실제 출력의 정확성·완전성이나 토큰 효율은 실행 결과 없이 점수화하지 않습니다.

### 스코어링 루브릭 (4차원 가중 평균)

> 점수는 실행 결과와 평가 근거가 있을 때만 계산합니다. 실행하지 않은 검사는 `NOT-RUN`입니다.
...
- **최저 통과**: 관측된 모든 차원과 assertion을 평가한 경우 가중 평균 >= 0.70
- **단일 차원 floor**: 어떤 차원도 0.50 미만이면 hard fail
- **신규 스킬**: 첫 평가에서 >= 0.75 필요. 실제 실행하지 않았다면 점수로 출시 판단을 대신하지 않음
...
테스트 후 작성하는 리포트 형식이다. 아래 수치와 판정은 형식 설명용이며 실제 측정값이 아니다:
...
- Mode: A/B Test
- Date: YYYY-MM-DD
- Host / model / configuration: <실제 실행 환경>
- Result: PASS / FAIL / NOT-RUN
- Evidence: <실행 명령 또는 앱 기록과 실제 출력 위치>
- Gaps: <실행하지 못한 검사>
...
**예시 1: 신규 스킬 A/B 테스트**
> "meta-skill-tester로 collab-sales-playbook 스킬 A/B 테스트해줘"

**예시 2: 회귀 테스트**
> "content-blog 스킬 수정했는데 회귀 테스트해줘"

**예시 3: 체인 테스트**
> "content-blog → ai-slop-reviewer 체인 테스트 실행해줘"
...
- 토큰 사용량은 대상 호스트가 해당 실행의 값을 제공할 때만 기록합니다. 제공하지 않으면 `미측정`으로 표시합니다.
- 자동화된 CI 통합은 추후 Phase 4에서 구현 예정입니다
- 테스트 프롬프트에 민감 정보(실제 고객명 등)를 포함하지 마세요
- 정적 문서 검토, 테스트 실행, 실제 데스크톱 앱 확인은 각각 구분해 보고합니다.

## 관련 스킬

| 스킬 | 관계 | 설명 |
|------|------|------|
| meta-skill-builder | before | 스킬 생성 후 테스트 실행 |
