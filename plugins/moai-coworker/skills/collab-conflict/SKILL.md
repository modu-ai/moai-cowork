...
- **일회성 vs 반복**: 한 번의 상황도 심각도와 안전을 살피고, 반복 행동은 기록과 함께 다룬다
...
| 세부 확인이 많은 상사 | 확인 요청이 잦음 | 보고 빈도와 자율 범위를 합의하고 부담을 확인(`collab-feedback-loop`) |
...
→ 확인 요청의 내용과 빈도 파악 → 보고 방식·자율 범위 합의 대화 설계
...
대응 스크립트·거절 멘트는 관찰한 행동만 적고 상대의 동기·성격을 단정하지 않았는지, 거절 문장이 짧고 자연스러운지 직접 검수한다. 아래 스킬은 현재 앱에 노출된 경우에만 추가로 사용한다.

- **moai-coworker:ai-slop-reviewer**: 노출된 경우 표현 추가 검수
- **moai-writer:korean-humanize**: 별도 설치돼 노출된 경우 한국어 문장 추가 윤문. 갈등 사실과 안전 안내는 보존

## 이 스킬을 사용하지 말아야 할 때

- **직장 내 괴롭힘·차별·폭언**: 소통 기술이 아니라 인사·고충처리·법적 절차의 영역이다.
- **공식 징계·인사 조치**: 평가·징계는 `moai-recruiter` 영역을 사용한다.
- **개인 멘탈·번아웃 관리**: 깊은 소진은 전문 상담·`moai-coworker`의 자기관리로 다룬다.

## 출처

직장 갈등 대응을 위한 일반적인 대화 구조다. 사용자 자료를 실제로 제공받은 경우에만 그 내용을 반영하고, 법적 판단이나 인사 조치가 필요하면 해당 절차로 연결한다.
