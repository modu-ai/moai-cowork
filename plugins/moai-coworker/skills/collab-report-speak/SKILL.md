...
| ② 근거 | 실제 확인된 사실과 자료 | "두 안의 비용과 일정 차이는 첨부 표에 있습니다." |
| ③ 리스크·대안 | 알아야 할 위험·다른 선택지 | "단, 외주 의존도가 올라갑니다." |
| ④ 요청·다음 단계 | 실제 결정이 필요한 내용 | "A안 진행 여부를 검토해 주세요." |
...
- **위험을 먼저 묻는 상대**: 확인된 위험과 가능한 대비책을 함께 제시하기.
...
4. **검증된 숫자·예시 활용**: 측정 자료가 있을 때만 수치를 쓰고, 예시는 실제인지 가상인지 밝히기
...
보고 스크립트는 수신자의 관계와 조직 문체에 맞춥니다. `references/kr-honorific-tone.md`는 선택 가능한 표현 예시이며, 직급만으로 말투나 서명을 확정하지 않습니다.
...
보고 스크립트·설명문은 결론과 요청이 먼저 보이는지, 수치·일정·책임자가 확인된 자료와 맞는지, 문장이 실제로 말하기 자연스러운지 직접 검수한다. 아래 스킬은 현재 앱에 노출된 경우에만 추가로 사용한다.

- **moai-coworker:ai-slop-reviewer**: 노출된 경우 표현 추가 검수
- **moai-writer:korean-humanize**: 별도 설치돼 노출된 경우 한국어 문장 추가 윤문. 보고 근거·요청은 보존

## 이 스킬을 사용하지 말아야 할 때

- **긴 문서·보고서 자체를 작성**: `moai-coworker:collab-productivity-report` 또는 `moai-coworker:collab-exec-summary`를 사용한다. 이 스킬은 말과 구조에 집중한다.
- **공식 성과평가 면담**: 평가·고과 피드백은 `moai-recruiter:hr-performance-review`를 사용한다.
- **마케팅 카피 작성**: 요청한 채널에 맞는 `moai-marketer` 스킬을 사용한다.

## References

| 파일 | 로드 조건 |
|------|-----------|
| references/kr-honorific-tone.md | 존댓말·톤 공유 규격 (이 스킬이 물리 소유). 수신자 위계별 종결어미·서명·요청/거절/사과/독촉 화법이 필요할 때 — 이메일·보고·CS 응대·협상·공지 스킬이 공동 참조 |

## 출처

결론 먼저 설명하는 일반적인 대화 구성 예시다. 사용자 자료는 실제로 제공된 경우에만 반영한다.
