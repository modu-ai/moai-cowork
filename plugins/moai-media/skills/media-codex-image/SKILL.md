...
description: |
  ...
  GPT Image 2.5를 정확히 지정한 요청은 moai-mcp-openai 도구로 생성합니다.
...
2. 사용자가 **GPT Image 2.5로 새 이미지 생성**을 정확히 지정했다면 `moai-mcp-openai` 연결의 `openai_image_generate` 도구를 확인한다. Flare 또는 Sunburst 중 사용자가 지정한 모델을 유지하고, 지정하지 않았다면 속도 우선은 Flare, 까다로운 품질 요구는 Sunburst를 권한다. 이 도구는 별도 OpenAI API 키와 API 과금이 필요하다. 생성 전에 그 사실과 선택 모델·크기를 알리고 유료 호출에 대한 명시적인 승인을 받는다. 도구나 키가 없으면 기본 도구로 조용히 바꾸지 않고 2.5 지정 생성은 미완료라고 알린다. API 키를 채팅이나 ChatGPT 로그인 토큰에서 받지 않는다. **원본 이미지를 넣는 2.5 편집 요청에는 이 생성 도구를 호출하지 않는다.** 해당 편집 경로는 아직 제공되지 않으므로 미지원이라고 알린다.
...
OpenAI의 [ChatGPT 데스크톱 이미지 생성 문서](https://learn.chatgpt.com/docs/image-generation)는 기본 도구의 모델을 `gpt-image-2`로 명시한다. [ChatGPT Images 2.5 안내](https://help.openai.com/en/articles/11084440)는 웹·모바일의 Images 기능을 설명한다. 따라서 데스크톱 기본 도구 결과에 2.5 모델명을 붙이지 않는다. 정확한 2.5 모델 지정은 `moai-mcp-openai`가 OpenAI Image API에 요청하는 별도 인증·과금 경로를 따른다. ChatGPT 로그인 토큰을 API 키처럼 사용하거나 `~/.codex/auth.json`을 읽어 넘기지 않는다.
...
| 정확한 GPT Image 2.5 이미지 한 장 생성 | `moai-mcp-openai`의 `openai_image_generate` (`gpt-image-2.5-flare` 또는 `gpt-image-2.5-sunburst`), 별도 API 인증·과금 필요 |
