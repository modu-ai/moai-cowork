...
2. **라이브 조회.** MCP는 `models_explore(action:'get')`, ChatGPT 공식 플러그인은 노출된 모델 상세 도구로 실제 제약을 가져온다. 3D·오디오는 모델별 파라미터 편차가 이미지·영상보다 크므로 이 단계를 건너뛰지 않는다.
3. **비용 프리플라이트.** 현재 연결의 해당 작업 견적 도구로 `credits`를 확인한다. 견적 도구가 없으면 비용을 추측하지 않고 생성을 보류한다. 3D의 텍스처·리깅·애니메이션은 옵션을 켠 상태의 비용을 확인한다.
