...
새로 만들지 않고 이미 있는 에셋을 바꾸는 경우다. 현재 연결에 해당 전용 도구가 있으면 스키마를 확인하고 **그 도구의 견적 모드로 먼저 비용을 조회**한다. 입력 에셋·변형 옵션·견적 크레딧·잔액을 보여주고 코어의 질문 채널로 명시적 승인을 받은 뒤에만 실제 잡을 제출한다. 견적 모드가 없거나 결과가 불명확하면 제출하지 않는다. 제출 뒤에는 반환된 잡 ID로 완료를 확인하고, 응답이 애매하면 기존 잡을 먼저 조회한다. 가격이나 결과가 다른 생성 경로보다 유리하다고 조회 없이 단정하지 않는다.
