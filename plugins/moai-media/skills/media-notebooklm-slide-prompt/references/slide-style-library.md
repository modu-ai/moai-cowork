...
> media-notebooklm-slide-prompt 스킬이 Phase 3(나노바나나 이미지 프롬프트 생성) 또는 Phase 2의 `[4] 톤 & 스타일` 블록 작성 시 참조하는 시각 스타일 카탈로그.
...
2. 아래 카테고리에서 후보 2~3개를 고른다. 필수 선택이면 직접 실행에서만 현재 앱의 질문 채널로 제시한다. 하위 에이전트에서는 질문 도구가 보여도 상위에 blocker를 반환하고, 직접 실행에 질문 채널이 없어도 같은 blocker를 반환한다. 선택 사항이면 추천 기본값을 밝히고 진행한다.
...
- ❌ 사용자 언어·발표 맥락을 무시하고 영문 키워드를 기계적으로 복사 — 의미와 어조 불일치
...
본 라이브러리는 공개된 옵시디언 publish 자료(이커머스 클래스 · 노트북 LM 슬라이드 가이드북)에서 49개 시각 스타일 분류를 참고해, MoAI 모두의 코워크에서 사용하기 위해 카테고리·매칭 규칙·일관성 가드를 재구성한 자료입니다. 영문 키워드는 스타일을 고르는 예시이며, 사용자 언어와 발표 맥락에 맞게 바꿔 써도 됩니다.
