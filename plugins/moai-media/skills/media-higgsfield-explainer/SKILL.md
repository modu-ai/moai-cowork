...
**실행 전 기능 확인:** 현재 세션에 설명 영상 프리셋 조회·해석 및 최종 블록 조립 도구가 모두 노출됐는지 확인한다. ChatGPT 공식 Higgsfield 플러그인에서 관측된 일반 이미지·영상·오디오 도구만으로 이 스킬의 서버 조립 절차를 재현할 수 있다고 가정하지 않는다. 필요한 도구가 없으면 생성 작업을 시작하지 않고 사용 가능한 연결과 빠진 기능을 보고한다.
