...
- **[HARD] 한국어 나레이션은 합성 전에 원문·발음·사실·민감정보·자연스러운 낭독을 검수한다.** 음성으로 굳은 뒤 고치면 할당량을 다시 쓴다. 아래 외부 스킬은 현재 앱에 설치·노출된 경우에만 추가로 사용한다.

```
  moai-coworker:ai-slop-reviewer     노출된 경우 문체 검수
→ moai-writer:korean-spell-check     노출되고 외부 전송 가능한 원고인 경우 맞춤법 제안
→ moai-writer:korean-humanize        노출된 경우 윤문과 최종 검수
```

  외부 스킬이 없어도 이 스킬에서 원문 대조, 수치·고유명사·발음·민감정보·문장 호흡을 확인한다. 수정이 있었다면 **문자 수를 다시 센다** — 이전 견적은 무효다.
...
→ 업로드 전 승인 게이트 (직접 실행은 질문 채널, 하위 에이전트는 상위에 승인서 blocker 반환)
...
[HARD] 이 스킬의 게이트는 **특정 도구 이름에 묶이지 않는다.** Claude의 `AskUserQuestion`과 ChatGPT Work의 질문 도구는 이름과 반환 방식이 다를 수 있다. 직접 실행에서는 현재 노출된 질문 도구로 승인서를 제시한다. `media-producer` 같은 하위 에이전트는 질문 도구가 보여도 승인서를 담은 blocker를 상위에 반환한다. 비동기 도구의 즉시 반환을 사용자의 승인으로 간주하지 않는다.
...
| 순위 | 경로 | 조건 |
|---|---|---|
| 1 | 직접 실행의 구조화 질문 도구 | 그 도구가 현재 세션에 노출돼 있을 때. 사용자의 명시적 응답을 기다림 |
| 2 | 구조화 blocker 반환 → 상위 오케스트레이터가 물어봄 | 하위 에이전트 실행이거나 직접 실행에 질문 도구가 없을 때 |
