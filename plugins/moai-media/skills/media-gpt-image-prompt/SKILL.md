...
이미 주어진 요청에서 생성인지 편집인지, 어떤 프리셋이 맞는지 확인합니다. 결과를 바꾸는 필수 정보가 비어 있으면 직접 실행에서만 현재 앱의 질문 채널을 사용합니다. 하위 에이전트에서는 질문 도구가 보여도 상위에 차단 보고를 반환하고, 직접 실행에 질문 채널이 없어도 필요한 입력을 명시한 차단 보고를 반환합니다. 편집이면 Round 2 대신 [편집 워크플로우](#편집-워크플로우)로 갑니다.
...
선택된 프리셋의 `presets/<name>.md`에 정의된 슬롯 중 결과에 필요한 것만 확인합니다. 직접 실행의 질문 채널이 있으면 그 도구의 선택지 형식을 씁니다. 하위 에이전트이거나 직접 실행에 채널이 없고 필수 정보가 빠졌다면 차단 보고에 필요한 입력을 적습니다. 이미 받은 정보는 다시 묻지 않습니다.
