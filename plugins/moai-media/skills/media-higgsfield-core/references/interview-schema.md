...
> 이 파일은 **수집할 슬롯을 정의**할 뿐, 도구 호출을 지시하지 않는다. 직접 대화 중인 실행은 현재 앱에서 질문하고, 사용자에게 물을 수 없는 하위 실행은 누락 슬롯을 상위에 전달한다.
...
- 이 참조 문서는 **수집할 슬롯(slots to collect)**을 서술한다. 사용자와 직접 대화 중이면 현재 앱의 구조화 질문 기능 또는 대화로 필요한 슬롯을 확인한다.
- 사용자에게 물을 수 없는 하위 실행은 누락 슬롯, 선택지, 재개 방법을 구조화된 blocker로 반환한다. 도구 이름이나 특정 런타임에 질문을 묶지 않는다.
