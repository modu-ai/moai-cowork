...
- **[HARD] 승인은 §승인 요청 계약의 경로로 받는다.** 현재 앱의 질문 채널이 있으면 승인서를 제시하고 명시적 답을 받는다. 질문 채널이 없으면 직접 대화 중이어도 승인에 필요한 입력을 담은 blocker를 반환한다.
...
이 파일은 수집할 슬롯을 정의한다. 현재 앱의 질문 채널이 있으면 빠진 슬롯을 확인한다. 질문 채널이 없으면 직접 대화 중에도 누락 슬롯과 선택지·재개 방법을 구조화된 blocker로 반환한다. 상세는 `references/interview-schema.md`.

## 승인 요청 계약 (런타임 중립)

[HARD] 승인에는 사용자에게 승인서를 그대로 보여주고 명시적 응답을 받는 질문 채널이 필요하다. 도구 이름은 런타임마다 다르므로 현재 앱에서 실제 노출된 질문 채널을 사용한다. 저장소 `AGENTS.md`의 question-channel 바인딩에 따라 채널이 없으면 대화 본문에 질문하지 않고 blocker를 반환한다.

| 순위 | 경로 | 조건 |
|---|---|---|
| 1 | 현재 런타임의 질문 채널 (`AskUserQuestion` 등) | 현재 세션에 노출돼 있을 때 |
| 2 | 구조화 blocker 반환 | 질문 채널이 없을 때. 직접 대화·하위 실행 모두 해당 |
