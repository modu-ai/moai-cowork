...
2. **Reason / Plan** — Choose the backend from the user's request and available app tools: ChatGPT native Images for ordinary ChatGPT image work, `moai-mcp-openai` for an exact GPT Image 2.5 request when separately authenticated, Higgsfield for explicit Higgsfield use, ElevenLabs for connected audio, and prompt-builder skills when only a prompt is requested. Identify credits or usage limits, aspect constraints, and brand anchors.
