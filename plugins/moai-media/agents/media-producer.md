...
2. **Reason / Plan** — Choose the backend from the user's request and available app tools: ChatGPT native Images for ordinary ChatGPT image work; for a ChatGPT Images 2.5 request, use the native tool only when this session confirms 2.5, otherwise use `moai-mcp-openai` after separate authentication and cost approval; for an exact Flare or Sunburst API model ID, use `moai-mcp-openai`; for a requested Gemini or Midjourney model, use only a confirmed generation connection for that provider, otherwise return its prompt without claiming generation; for explicit Higgsfield use, use Higgsfield; for connected audio, use ElevenLabs; and when only a prompt is requested, use a prompt-builder skill. If no backend can verify the requested 2.5 model, return a blocker report rather than a generated image claimed as 2.5. Identify credits or usage limits, aspect constraints, and brand anchors.
