...
요약 표의 분류는 canvas의 RGB 채널 산술평균에 따른 과거 카탈로그 라벨이다. `warm`은 색온도가 따뜻하다는 뜻이 아니라 중간 밝기 구간 이름이며 WCAG 상대휘도 계산도 아니다. `canvas-light`·`canvas-dark`를 함께 가진 시스템은 용도를 먼저 골라야 하므로 이 표의 단일 캔버스 값을 렌더 토큰으로 바로 사용하지 않는다.
표의 `폰트` 열은 원본 사이트를 조사할 때 기록한 참고 서체명이다. 배포에 쓰는 기본 서체는 각 시스템 파일의 `typography.*.fontFamily`를 따른다. 원본 서체를 쓰려면 해당 용도의 사용권을 별도로 확인한다.
...
canvas hex의 R+G+B 산술평균(과거 카탈로그 분류):

- 평균 < 100 → **dark** (캔버스가 매우 어두움, 본문 white)

- 100 ≤ 평균 < 232 → **warm** (레거시 라벨; 중간 밝기, 색온도 아님)
