...
description: |
  Resend's source marketing surfaces sit on a near-pure black canvas with
  off-white text and editorial Domaine Display headlines. The portable tokens
  use OFL DM Serif Display for 76px–96px headlines, Inter for body and UI, and
  Geist Mono for code; the source's Domaine Display and ABC Favorit remain
  reference fonts pending usage rights. Surfaces rely on subtle 6–9% opacity gradient
...
components:
  ...
  code-tab-active:
    backgroundColor: "{colors.surface-card}"
    textColor: "{colors.ink}"
    borderBottom: "2px solid {colors.control-border}"
    typography: "{typography.code-md}"
    rounded: "{rounded.sm}"
    padding: 6px 12px
  email-mockup:
...
Every page in the reference set opens on `{colors.canvas}` (`#000000`). The source uses a 96px Domaine Display serif headline ("Email for developers", "Email reimagined"); the portable tokens use DM Serif Display with an operating-system serif fallback. That single typographic
decision sets the brand tone: confident, considered, slightly literary, and
priced on quality rather than novelty.

The source uses **ABC Favorit** for marketing prose. The portable tokens use **Inter** for prose and UI labels, while code blocks use **Geist Mono** with platform fallbacks inside `{components.code-window}` shells with decorative traffic-light dots. Surface depth is built almost entirely from translucent white —
...
grid → black footer. The dark page occasionally uses the light `{components.email-mockup-light}` inset for rendered email examples, framed like print in a magazine.

**Key Characteristics:**
- Pure black canvas (`{colors.canvas}` — `#000000`) on every public page; off-white text (`{colors.ink}` — `#fcfdff`) carries the full read.
- Portable type system: **DM Serif Display** at 76–96px for hero headlines, **Inter** for body/UI, and **Geist Mono** for code; Domaine Display and ABC Favorit remain source references.
- Four accent glow tokens used only as low-opacity atmospheric washes (`{colors.accent-orange}`, `{colors.accent-blue}`, `{colors.accent-green}`, `{colors.accent-red}`, `{colors.accent-yellow}`) — never as buttons or solid surfaces.
...
- Translucent white hairlines remain decorative; `{colors.control-border}` supplies visible card and input boundaries without a shadow.
- `{components.button-primary}` is a small white rectangle with black text — counterintuitive contrast that becomes the page's brightest pixel and works as a single visual anchor.

## Colors

### Brand & Accent
- **Primary White** (`{colors.primary}` — `#fcfdff`): the brand's de facto accent. Reserved for `{components.button-primary}` (white pill on black canvas), display headlines, and the active text colour. White is the loudest possible colour on this canvas — that's the signature.
...
- **Surface Light** (`{colors.surface-light}` — `#f1f7fe`): a subtle blue-tinted off-white used as the active/pressed state of `{components.button-primary}`.
...
- **Hairline** (`{colors.hairline}` — `rgba(255,255,255,0.06)`): decorative row divider only.
- **Hairline Strong** (`{colors.hairline-strong}` — `rgba(255,255,255,0.14)`): decorative separation only; too faint for required control boundaries.
- **Control Border** (`{colors.control-border}` — `#707070`): visible card, button, code-window, and input boundaries on dark surfaces.
...
- **On-Light** (`{colors.on-light}` — `#000000`): text inside `{components.email-mockup-light}`.
...
- **Link** (`{colors.link}` — `#3b9eff`): underlined links on dark surfaces.
- **Light Link** (`{colors.link-light}` — `#005a9c`): underlined links inside `{components.email-mockup-light}`.

## Typography

### Font Family

The source design uses proprietary Domaine Display and ABC Favorit. The portable tokens use [OFL DM Serif Display](https://github.com/google/fonts/blob/main/ofl/dmserifdisplay/METADATA.pb) for display, [OFL Inter](https://github.com/google/fonts/blob/main/ofl/inter/METADATA.pb) for body/UI, and [OFL Geist Mono](https://github.com/google/fonts/blob/main/ofl/geistmono/METADATA.pb) for code. Load fonts explicitly when available; the tokens include macOS, Windows, and Linux fallbacks. Use the proprietary source fonts only after rights are confirmed. Source-specific stylistic-set features are omitted from the portable defaults.

### Hierarchy

| Token | Size | Weight | Line Height | Letter Spacing | Use |
|---|---|---|---|---|---|
| `{typography.display-xxl}` | 96px | 400 | 1.15 | -0.96px | Home hero ("Email for developers"). One per page. |
| `{typography.display-xl}` | 76.8px | 400 | 1.15 | -0.768px | Section openers ("Email reimagined", "Available today"). |
| `{typography.display-lg}` | 56px | 400 | 1.2 | -1.2px | Inter display sub-titles. |
...
| `{typography.body-md}` | 16px | 400 | 1.5 | 0 | Inter body. |
...
| `{typography.caption-emph}` | 14px | 600 | 1.15 | 0 | Emphatic small caption — Inter stack. |
| `{typography.code-md}` | 13px | 400 | 1.6 | 0 | Code blocks, inline code. |

### Principles
- Display tokens use `lineHeight: 1.15` with measured tracking; check mixed-script wrapping and clipping after fonts load.
...
- Proprietary stylistic sets are reference details; portable tokens leave font features at their defaults. Check code legibility in the loaded Geist Mono stack.
- Inline links need an underline and a visible keyboard focus state; `{typography.button-sm}` is reserved for small controls.

### Note on Font Substitutes

The portable fonts use the line heights and tracking specified in YAML. Do not force source-only stylistic sets onto DM Serif Display or Inter; recheck real glyph coverage and line breaks.
...
- Card internal padding: `{spacing.xxl}` (32px) on `{components.feature-card}`, `{components.pricing-tier}`, and `{components.code-window}`.

### Grid & Container
- **Max content width** ≈ 1200px on body sections.
- **Feature grid**: 3 columns at desktop, 2 at tablet, 1 at mobile.
- **Pricing**: 3-tier grid centred at desktop; centre tier promotes to `{components.pricing-tier-featured}` (one-step-elevated surface).
- **Code-story splits**: a 2-up split — narrative copy left, `{components.code-window}` right — collapsing to stacked at < 1024px.
- **Email mockup band**: a single `{components.email-mockup-light}` card (640px max width) centred in the dark canvas with generous vertical padding.

### Whitespace Philosophy
- Whitespace is editorial and generous — full-bleed sections breathe at 96–128px so display headlines have room to register at scale.
- Inside cards, padding stays at 32px so feature copy and code wells have a consistent rhythm with the outer grid.
- Decorative hairlines separate rows, while `{colors.control-border}` makes required component edges visible on the dark canvas.
...
| 1 — surface card | `{colors.surface-card}` (`#0a0a0c`) + 1px `{colors.control-border}` | Bordered feature cards, pricing tiers, form inputs. |
| 2 — elevated | `{colors.surface-elevated}` (`#101012`) + 1px `{colors.control-border}` | Featured pricing tier, ghost button. |
| 3 — code well | `{colors.surface-deep}` (`#06060a`) + 1px `{colors.control-border}` | Code window, terminal shells. |
| 4 — atmospheric glow | Low-opacity radial gradient (the accent glow tokens) anchored at section top | Section openers ("Integrate this weekend", "Email reimagined"). |
...
- **Code window traffic lights** — `{components.code-window}` shells include a row of three coloured dots (red `{colors.accent-red}`, yellow `{colors.accent-yellow}`, green `{colors.accent-green}`) at the top — the only place all three semantic colours appear together as solid surfaces.
...
- Background `{colors.surface-elevated}`, label `{colors.ink}`, 1px `{colors.control-border}`, type `{typography.button-md}`, `rounded: {rounded.md}`, height 36px.
- Equal-weight secondary action paired with `{components.button-primary}`.

**`button-outline`** — outlined CTA
- Background `{colors.canvas}`, label `{colors.ink}`, 1px `{colors.control-border}`, type `{typography.button-md}`, `rounded: {rounded.md}`, height 36px.
...
- Used only on the home page hero band; carries the 96px DM Serif Display headline and a single `{components.button-primary}` CTA. No photography, no atmospheric glow inside the hero itself — the glow appears on the section that follows.
...
- Background `{colors.surface-card}`, text `{colors.ink}`, 1px `{colors.control-border}`, type `{typography.body-md}`, `rounded: {rounded.lg}`, padding `{spacing.xxl}`.
- Used when feature cards sit close together and need explicit separation.

**`pricing-tier`** — pricing tier card
- Background `{colors.surface-card}`, text `{colors.ink}`, 1px `{colors.control-border}`, type `{typography.body-md}`, `rounded: {rounded.lg}`, padding `{spacing.xxl}` (32px).
- Tier name in `{typography.heading-md}` + price in `{typography.display-lg}` (Inter, 56px).

**`pricing-tier-featured`** — recommended tier
- Background `{colors.surface-elevated}`, text `{colors.ink}`, 1px `{colors.control-border}`, type `{typography.body-md}`, `rounded: {rounded.lg}`, padding `{spacing.xxl}`.
- Centre tier elevated by surface luminance, not by colour.

**`code-window`** — code well
- Background `{colors.surface-deep}`, text `{colors.body}`, type `{typography.code-md}`, 1px `{colors.control-border}`, `rounded: {rounded.lg}`, padding `{spacing.xl}` (24px).
...
- `{components.code-tab-active}` uses `{colors.ink}` text and a 2px `{colors.control-border}` underline; the selected language also needs a programmatic state.

**`email-mockup`** — email-card inset
- `{components.email-mockup}` is the dark variant with `{colors.surface-card}` and `{colors.ink}`. `{components.email-mockup-light}` is the light inset with `{colors.surface-light}` and `{colors.on-light}`; use `{components.link-inline-light}` for links inside it. Choose the variant before placing text.
- Used in the "Beyond experience" band to demonstrate rendered email output.

### Inputs & Forms

**`text-input`** — default input
- Background `{colors.surface-card}`, text `{colors.ink}`, type `{typography.body-sm}`, 1px `{colors.control-border}`, `rounded: {rounded.md}`, padding `10px 14px`, height 40px.
- Sign-up and waitlist email fields. `{components.text-input-focused}` uses a 2px light border and outline; check keyboard focus in the rendered interface.
...
- Left: wordmark logo. Centre: top-level nav ("Features", "Pricing", "Docs", "Customers"). Right: "Sign in" link + `{components.button-primary}`.
...
- Pill chips set in a horizontal row above content (e.g. on the customers index), `{components.sub-nav-pill}` styling.
...
- Reserve `{components.button-primary}` (white pill) as the only solid bright surface. One per viewport at most.
- Set portable hero headlines in **DM Serif Display** at 76–96px with `lineHeight: 1.15`; treat Domaine Display and its stylistic sets as source references only.
- Use the **Inter** stack for marketing body and UI labels, and the **Geist Mono** stack for code.
- Build elevation from translucent-white hairlines, not drop shadows.
- Use the accent glow tokens as low-opacity radial atmospheric washes — never as solid surfaces.
- Set buttons and inputs to `{rounded.md}` (8px); cards and code wells to `{rounded.lg}` (12px); pills and avatars to `{rounded.full}`.
- Use the light email-mockup inset sparingly — it's the only deliberately-light surface and should feel like a print pull-quote.
...
- Don't use proprietary source fonts as shipping defaults without confirmed rights.
- Use the Geist Mono stack for code; inline code can use a compact dark chip outside `{components.code-window}`.
...
### Touch Targets
- The YAML lists 36px buttons and a 40px input; the sub-nav pill has no fixed height. Measure rendered target size and spacing on each layout, then enlarge smaller targets where needed for WCAG 2.2 criteria. Check keyboard focus and operation in the running interface.
...
1. Focus on ONE component at a time. Most surfaces share `{colors.surface-card}` or `{colors.surface-elevated}` with `{rounded.lg}` — only the role-specific tokens (`{colors.primary}`, `{components.code-window}`) shift between variants.
...
6. Keep `{colors.primary}` (white) scarce — if more than one solid white surface appears per viewport, ask whether one should drop to `{components.button-ghost}` instead.

## Known Gaps

- Current Resend copy, pricing, service status, testimonials, photos, logo, and reuse rights have not been checked. Verify current sources and permission before publication.
- Font loading and glyph fallback, actual border and focus rendering, target sizes, and desktop behavior on macOS/Windows/Linux have not been observed.

- Pressed and active states are documented for `button-primary-pressed` and `code-tab-active`; other controls still need explicit focus and active-state testing in the rendered interface.
