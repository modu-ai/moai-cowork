...
v0.2.0 대비 변경: 11개 스킬 그룹 → 15개 독립 플러그인 아키텍처로 전환.
...
사용자의 자연어 요청을 분석하여 **15개 독립 플러그인** 중 적합한 플러그인으로 라우팅.
...
| 변경 사항 | 내용 |
|---------|------|
| 라우팅 단위 변경 | 11개 스킬 그룹 → 15개 독립 플러그인 |
| router.md | 15 plugin 매핑 테이블로 재작성 |
| 아키텍처 | 3계층 플러그인 구조 반영 |
...
```
계층 1: 플러그인 (Read-Only) — 15개 플러그인 + 레퍼런스 + 스크립트
