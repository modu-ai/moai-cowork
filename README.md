...
> **크레딧**: 본 하네스 카탈로그는 [황민호(Minho Hwang)](https://www.facebook.com/rev.minho) 님의 [harness-100](https://github.com/revfactory/harness-100) 프로젝트의 하네스 지침을 참고하여 Claude Cowork 플러그인에 맞게 재작성하였습니다. 훌륭한 하네스 설계와 도메인 전문 지식을 공유해 주신 황민호 님께 깊이 감사드립니다. 🙏
...
MoAI는 **전세계 모든 국가**를 지원합니다. `/moai init` 실행 시 근무 국가를 입력하면, 해당 국가의 규제·문화·비즈니스 관행 데이터를 자동으로 수집합니다.

| 방식 | 대상 | 설명 |
|------|------|------|
| **내장 데이터** | 한국(KR) | 세법, 노동법, 데이터보호법, 비즈니스 관행, 형식 표준이 플러그인에 포함 |
| **웹검색 동적 수집** | 한국 외 전세계 | `/moai init` 시 근무 국가 입력 → 웹검색으로 5개 카테고리 데이터 실시간 수집 |

**수집 항목** (모든 국가 공통):
1. 세법 기본사항 (VAT/GST, 소득세, 법인세, 주요 신고일정)
2. 노동법 핵심 (근무시간, 최저임금, 유급휴가, 사회보험)
3. 데이터보호법 (주요 법률, 동의 요건, 보관기한)
4. 비즈니스 관행 (호칭, 회의 문화, 의사결정 방식)
5. 형식 표준 (날짜, 시간, 통화, 숫자, 주소 형식)

수집된 데이터는 `.moai/locale-context.md`에 저장되며, 하네스 실행 시 자동으로 참조됩니다.
...
2. **로케일 개선** — `references/locale/kr/` 한국 내장 데이터 최신화 및 웹검색 수집 로직 개선
...
| 버전 | 내용 |
|------|------|
| **V.0.1.0** (현재) | 100개 하네스 (ko+en), 코어 프로토콜, 전세계 로케일 (웹검색 기반), 17개 언어 README |
| **V.0.2.0** | 하네스 추천 엔진, 내장 로케일 국가 확대 |
